...
- Traditional build is (with default profile) :
  ...
  - Example for RedHat / PPC64-LE:

	cp /usr/bin/node node-repo/node/node

	mvn -B clean install package rpm:rpm -Pnode-local -DnewVersion=${AMBARI_VERSION} -DskipTests -Dpython.ver="python > 2.6" -Preplaceurl -l mvn.compile-install.res &
